...
We know that when we draw from a population that is normally distributed with mean $\mu$ and standard deviation $\sigma$, the sampling distribution is $\bar X \sim Normal(\mu ,\sigma/ \sqrt n)$ (provided the draws are iid)
...
As we may expect from the results seen above, there is a very slim chance that we will see a value that extreme.
